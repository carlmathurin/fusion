...
def get_time_from_gout(swap_endian=False):
   ...
   f = open('/scratch/01658/drhatch/dna_out/g_out.dat','rb')
...
def get_entropy_hermite(g_in,kzind=-1,include_kz0=True):
   ...
   if kzind==-1:
       ...
       g_kz=g_in[:,:,kzind,:]
       entropy=2.0*\
       np.real(0.5*np.pi*np.sum(np.sum(np.conj(g_kz[1:,:,:])*g_kz[1:,:,:],axis=0),axis=0))
       entropy=entropy+np.real(0.5*np.pi*np.sum(np.conj(g_kz[0,:,:])*g_kz[0,:,:],axis=0))
   return entropy

def get_entropy_hermite2(g_in,i,j,k):
    g_perp = g_in[i,j,kzind,:]

    entropy=2.0*np.real(
        0.5 * np.pi * (np.conj(g_perp[:,:])*g_perp[:,:])
                )
