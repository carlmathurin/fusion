...
par = {'omt':10.0, 'omn':1.0, 'Ti0Te':1.0, 'kxmin':0.05, 'kxmax0':1.55, 'kymin':0.05, 'kymax0':1.55, 'kzmin':0.1, 'kzmax0':3.1, 'nkx0':32, 'nky0':64, 'nkz0':64,
       'nv0':48, 'nh0':1, 'nspec':1, 'hyp_x':1.75, 'hyp_y':1.75, 'hyp_z':0.0, 'hypx_order':8, 'hypy_order':8, 'hypz_order':8, 'hyp_v':2.0, 'hypv_order':8, 'hyp_conv':1.0,
       'num_k_hyp_conv':1, 'hyp_conv_ky':False, 'np_herm':24, 'np_kz':1, 'np_hank':1, 'np_spec':1, 'hyp_nu':0.1, 'nuno_closure':True, 'em_conserve':True}
...
def matrix(kx,ky,kz,Gam0,nu):
    ...
    mat[0,0] =  1j*ky*factor*( par['omt']*k_perp2/2.0 - par['omn'] )
    mat[2,0] = -1j*par['omt']*ky*factor/np.sqrt(2.0)
    mat[1,0] = -1j*kz*factor
    ...
    for i in range(par['nv0']-1):       # be careful here since we miss the last element in the lower right corner
        if ( ( np.abs(ky)-par['kymin']*par['num_k_hyp_conv'] < 1.0e-6 and np.abs(ky) > 1.0e-6 and par['hyp_conv_ky'] ) or ( np.abs(kz)-par['kzmin']*par['num_k_hyp_conv'] < 1.0e-6 ) ):
            ...
            diag[i,i] = nu*float(i) + par['hyp_v']*(float(i)/float(par['nv0']-1))**par['hypv_order']\
                      ...
                      + par['hyp_y']*(ky/par['kymax0'])**par['hypy_order']\
                      + par['hyp_z']*(kz/par['kzmax0'])**par['hypz_order']
        phase_mix[i,i+1] = np.sqrt(float(i+1))
        phase_mix[i+1,i] = np.sqrt(float(i+1))
